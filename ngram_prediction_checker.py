...
rows = []
with open("one_customer.csv", 'r') as file:
...
def learn():
    ...
    step_size = 0.1
    swing_limit = 10
    while weight_length + weight_lag + weight_frequency + weight_recency < 4.5:
        auto_count = 0
        switch = 0
        last_prob = 0
        weight_adder = 0
...
for row in rows:
    if len(row) > 1:
        ...
        testing_data = row[70:90]
        print("Proportion of purchases in sample:", training_data.count("1")/training_data.count("2"))
        if (training_data.count("1")/training_data.count("2")) > 0:
            tester = []
            import time
